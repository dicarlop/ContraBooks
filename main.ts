...
require('source-map-support').install({ handleUncaughtException: false, environment: 'node' });
...
export class Main {
  title='ContraBooks'; icon:string; winURL=''; checkedForUpdate=false; mainWindow:BrowserWindow|null=null; templateDesignerWindow:BrowserWindow|null=null; WIDTH=1200; HEIGHT=process.platform==='win32'?826:800;
  constructor(){this.icon=this.getWindowIcon();if(this.isDevelopment)autoUpdater.logger=console;app.commandLine.appendSwitch('disable-http2');autoUpdater.requestHeaders={'Cache-Control':'no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0'};this.registerListeners();if(this.isMac&&this.isDevelopment)app.dock?.setIcon(this.icon);}
  get isDevelopment(){return process.env.NODE_ENV==='development'} get isTest(){return !!process.env.IS_TEST} get isMac(){return process.platform==='darwin'} get isLinux(){return process.platform==='linux'} get isWindows(){return process.platform==='win32'}
  getWindowIcon(){if(this.isLinux)return path.join(__dirname,'icons','512x512.png');if(this.isMac&&this.isDevelopment)return path.resolve('./build/icon.png');const iconPath=path.join(app.getPath('userData'),'contrabooks-cb.ico');try{if(!fs.existsSync(iconPath))fs.writeFileSync(iconPath,Buffer.from(WINDOWS_CB_ICON,'base64'));}catch(err){emitMainProcessError(err);}return iconPath;}
  ...
  async createTemplateDesignerWindow(name:string){if(this.templateDesignerWindow&&!this.templateDesignerWindow.isDestroyed()){this.templateDesignerWindow.focus();this.templateDesignerWindow.webContents.send('template-designer:navigate',name);return;}
    const preload=path.join(__dirname,'main','preload.js');const options:BrowserWindowConstructorOptions={width:1540,height:960,minWidth:1180,minHeight:760,title:`${name} — Template Designer`,titleBarStyle:'hidden',trafficLightPosition:{x:16,y:16},webPreferences:{contextIsolation:true,nodeIntegration:false,sandbox:false,preload},autoHideMenuBar:true,frame:!this.isMac,resizable:true,icon:this.icon,show:false};
    this.templateDesignerWindow=new BrowserWindow(options);this.templateDesignerWindow.on('closed',()=>{this.templateDesignerWindow=null;});const base=this.isDevelopment?(this.winURL||'http://0.0.0.0:6969/') : pathToFileURL(path.join(__dirname,'src','index.html')).toString();const separator=base.includes('?')?'&':'?';await this.templateDesignerWindow.loadURL(`${base}${separator}templateDesigner=${encodeURIComponent(name)}`);this.templateDesignerWindow.once('ready-to-show',()=>this.templateDesignerWindow?.show());}
