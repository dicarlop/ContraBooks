      ...
      <main class="dashboard-content">
        ...
        <section class="two-col middle">
          <article class="card panel">
            ...
            <button
              ...
              :key="row.id"
              class="table-row tx-cols"
              type="button"
              @click="row.path ? routeTo(row.path) : undefined"
            >
              <span>{{ row.date }}</span>
              <span>{{ row.description }}</span>
              <strong :class="row.negative ? 'negative' : 'positive'">{{ money(row.amount) }}</strong>
            </button>
          </article>
          ...
          <article class="card panel cashflow">
            <div class="panel-title">
              ...
              <select aria-label="Cashflow period">
                <option>Last 6 Months</option>
                <option>This Year</option>
              </select>
            </div>
            <div class="legend">
              <span><i class="bill"></i>Bills</span>
              <span><i class="expense"></i>Expenses</span>
              <span><i class="cash"></i>Cashflow</span>
            </div>
            <div class="chart">
              <div class="axis">
                <span>$12,000</span>
              ...
              <div class="chart-area">
                ...
                <div v-for="bar in cashFlowBars" :key="bar.label" class="month">
                  <div>
                    <i class="bill" :style="{ height: `${bar.bill}%` }"></i>
                    <i class="expense" :style="{ height: `${bar.expense}%` }"></i>
                  </div>
                  <span>{{ bar.label }}</span>
                </div>
                <div class="line">
                  <i
                    v-for="(bar, index) in cashFlowBars"
                    :key="index"
                    :style="{ left: `${8 + index * 17}%`, bottom: `${bar.line}%` }"
                  ></i>
                </div>
              </div>
            </div>
        ...
        <section class="two-col bottom">
          <article class="card panel list">
            <div class="panel-title">
              <h2><feather-icon name="file-text" />Financial Reports</h2>
              <button type="button" @click="routeTo('/report/ProfitAndLoss')">View All</button>
            </div>
            <div class="table-head report-cols">
              <span>Date</span>
              <span>Description</span>
              <span>Amount</span>
            </div>
            <div v-for="row in financialRows" :key="row.date + row.description" class="table-row report-cols">
              <span>{{ row.date }}</span>
              <span>{{ row.description }}</span>
              <strong :class="row.negative ? 'negative' : 'positive'">{{ money(row.amount) }}</strong>
            </div>
          ...
          <article class="card panel list">
            <div class="panel-title">
              <h2><feather-icon name="calendar" />Upcoming Tasks</h2>
              <button type="button">View All</button>
            </div>
            <div class="table-head task-cols">
              <span>Task</span>
              <span>Due Date</span>
            </div>
            <div v-for="task in tasks" :key="task.task" class="table-row task-cols">
              <span>{{ task.task }}</span>
              <span>{{ task.date }}</span>
            </div>
          </article>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      cashFlowBars: [
        { label: 'Apr', bill: 55, expense: 46, line: 37 },
        { label: 'May', bill: 51, expense: 55, line: 43 },
        { label: 'Jun', bill: 62, expense: 64, line: 42 },
        { label: 'Jul', bill: 69, expense: 73, line: 50 },
        { label: 'Aug', bill: 91, expense: 84, line: 50 },
        { label: 'Sep', bill: 100, expense: 82, line: 55 },
      ] as Bar[],
      financialRows: [
        { date: 'Sep 18, 2025', description: 'Customer Payment', amount: 1250 },
        { date: 'Sep 14, 2025', description: 'Bill Payment', amount: 845.2, negative: true },
      ...
      tasks: [
        { task: 'Send invoice to ABC Corp', date: '2025-09-16' },
        { task: 'Pay Office Supplies bill', date: '2025-09-17' },
        { task: 'Reconcile Bank Account', date: '2025-09-18' },
        { task: 'Prepare monthly reports', date: '2025-09-20' },
  ...
  methods: {
    ...
    syncTheme() {
      this.localDarkMode = localStorage.getItem('contrabooks-theme') === 'dark';
...
@media(max-width:850px){.dashboard-content{height:auto;min-height:100%;grid-template-rows:auto auto auto auto auto;overflow:auto}.dashboard-scroll{overflow:auto}.kpis,.middle,.bottom{grid-template-columns:1fr 1fr}.kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.company-button,.user-copy{display:none}.workflow{grid-template-columns:1fr 1fr}}
</style>
