<template>
  <aside class="sidebar-shell relative flex h-full min-h-0 w-full flex-col overflow-hidden text-white" :class="{ 'window-drag': platform !== 'Windows' }">
    <div class="sidebar-brand window-no-drag shrink-0">
      <img :src="logoUrl" alt="ContraBooks — Simple. Powerful. Yours." class="sidebar-logo" draggable="false" />
    </div>
    ...
    <nav class="window-no-drag min-h-0 flex-1 overflow-y-auto px-3 pb-4 no-scrollbar" aria-label="Main navigation">
      ...
      <button v-for="item in quickLinks" :key="item.label" class="sidebar-item quick-item" :class="isActive(item.route) ? 'sidebar-active' : 'sidebar-inactive'" type="button" @click="routeTo(item.route)">
        <feather-icon :name="item.icon" class="nav-icon" />
        <span class="truncate">{{ item.label }}</span>
      </button>

      <button class="sidebar-item quick-item" :class="$route.path === '/backup-wizard' ? 'sidebar-active' : 'sidebar-inactive'" type="button" @click="routeTo('/backup-wizard')">
        <feather-icon name="shield" class="nav-icon" />
    ...
    <button class="collapse-button" aria-label="Collapse sidebar" type="button" @click="() => toggleSidebar()">
  ...
  data() {
    return {
      ...
      navigation: [
        ...
        { label: t`Transactions`, icon: 'dollar-sign', route: '/list/Payment' },
...
.sidebar-logo { display:block; width:214px; height:auto; max-width:100%; }
...
.sidebar-divider { height:1px; margin:14px 8px 12px; background:rgba(255,255,255,.12); }
.quick-heading { display:flex; align-items:center; justify-content:space-between; height:28px; padding:0 12px; color:#A9C7DB; font-size:12px; font-weight:600; }
...
.collapse-button svg { width:15px !important; height:15px !important; }
</style>
