...
export async function initializeInstance(
  ...
  if (isNew) {
    ...
    countryCode = await fyo.db.createNewDatabase(dbPath, countryCode, unlockKey);
  } else if (!fyo.db.isConnected) {
    countryCode = await fyo.db.connectToDatabase(dbPath, undefined, unlockKey);
  }
...
async function setSingles(fyo: Fyo) {
  for (const schema of Object.values(fyo.schemaMap)) {
    if (!schema?.isSingle || schema.name === ModelNameEnum.SetupWizard) continue;
...
async function checkSingleLinks(fyo: Fyo) {
  const linkFields = Object.values(fyo.db.schemaMap)
    ...
    .map((field) => ({ fieldKey: `${field.schemaName!}.${field.fieldname}`, target: (field as TargetField).target }));
  ...
  for (const { name, fieldname, parent, value } of singleValues) {
    ...
    if (!linkFieldsMap[fieldKey]) continue;
    const { target } = linkFieldsMap[fieldKey];
    if (typeof value !== 'string' || !value || !target) continue;
    exists[target] ??= {};
    if (exists[target][value] === undefined) exists[target][value] = await fyo.db.exists(target, value);
    if (exists[target][value]) continue;
    await fyo.db.delete('SingleValue', name);
  }
}

async function setCreds(fyo: Fyo) {
  const email = (await fyo.getValue(ModelNameEnum.AccountingSettings, 'email')) as string | undefined;
  const user = fyo.auth.user;
  fyo.auth.user = email ?? user;
}

async function setVersion(fyo: Fyo) {
  const version = (await fyo.getValue(ModelNameEnum.SystemSettings, 'version')) as string | undefined;
...
async function setInstanceId(fyo: Fyo) {
  const systemSettings = await fyo.doc.getDoc(ModelNameEnum.SystemSettings);
  if (!systemSettings.instanceId) await systemSettings.setAndSync('instanceId', getRandomString());
  fyo.store.instanceId = (await fyo.getValue(ModelNameEnum.SystemSettings, 'instanceId')) as string;
}

export async function setCurrencySymbols(fyo: Fyo) {
  const currencies = (await fyo.db.getAll(ModelNameEnum.Currency, { fields: ['name', 'symbol'] })) as { name: string; symbol: string }[];
  fyo.currencySymbols = getValueMapFromList(currencies, 'name', 'symbol') as Record<string, string | undefined>;
}
...
async function setOpenCount(fyo: Fyo) {
  ...
  if (typeof openCount !== 'number') openCount = getOpenCountFromFiles(fyo);
...
function getOpenCountFromFiles(fyo: Fyo) {
  ...
  for (const file of configFile) {
    if (file.id === fyo.singles.SystemSettings?.instanceId) return file.openCount ?? 0;
  }
  return null;
}
