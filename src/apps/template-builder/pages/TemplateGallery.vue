          ...
          <article class="invoice-paper" :class="[`paper-${selectedPreset.toLowerCase()}`, `font-${options.font.toLowerCase()}`]">
            <div class="invoice-head" :class="`accent-${options.color}`">
              ...
              <div class="invoice-meta">
                <h3 v-if="field('Default Title')?.screen !== false">{{ field('Default Title')?.title || 'INVOICE' }}</h3>
                <span v-if="options.status && field('Default Title')?.screen !== false" class="status-pill" :class="{ 'past-due': options.pastDue }">{{ options.pastDue ? 'PAST DUE' : 'OPEN' }}</span>
                <div v-if="field('Invoice Number')?.screen !== false"># 10005</div>
                <div v-if="field('Date')?.screen !== false">September 16, 2026</div>
              </div>
            </div>

            <div class="bill-row">
              <div v-if="field('Bill To')?.screen !== false"><small>{{ field('Bill To')?.title || 'Bill To' }}</small><strong>Northstar Design Studio</strong><span>123 Wellington Street</span><span>Montreal, QC</span></div>
              <div v-if="field('Ship To')?.screen !== false" class="ship-box"><small>{{ field('Ship To')?.title || 'Ship To' }}</small><span>456 Saint-Catherine Street</span><span>Montreal, QC</span></div>
              <div class="due-box"><small>Amount Due</small><strong>$1,250.00</strong></div>
            </div>
            <div class="invoice-references">
              <span v-if="field('P.O. No.')?.screen !== false"><strong>{{ field('P.O. No.')?.title || 'P.O. No.' }}</strong> PO-2026-104</span>
              <span v-if="field('Terms')?.screen !== false"><strong>{{ field('Terms')?.title || 'Terms' }}</strong> Net 30</span>
              <span v-if="field('Due Date')?.screen !== false"><strong>{{ field('Due Date')?.title || 'Due Date' }}</strong> October 16, 2026</span>
            </div>

            <table>
              <thead><tr>
                <th v-if="field('Description')?.screen !== false">{{ field('Description')?.title || 'Description' }}</th>
                <th v-if="field('Quantity')?.screen !== false">{{ field('Quantity')?.title || 'Quantity' }}</th>
                <th v-if="field('Rate')?.screen !== false">{{ field('Rate')?.title || 'Rate' }}</th>
                <th v-if="field('Amount')?.screen !== false">{{ field('Amount')?.title || 'Amount' }}</th>
              </tr></thead>
              <tbody><tr>
                <td v-if="field('Description')?.screen !== false">Business Consulting</td>
                <td v-if="field('Quantity')?.screen !== false">2</td>
                <td v-if="field('Rate')?.screen !== false">$185.00</td>
                <td v-if="field('Amount')?.screen !== false">$370.00</td>
              </tr><tr>
                <td v-if="field('Description')?.screen !== false">Implementation Services</td>
                <td v-if="field('Quantity')?.screen !== false">1</td>
                <td v-if="field('Rate')?.screen !== false">$325.00</td>
                <td v-if="field('Amount')?.screen !== false">$325.00</td>
              </tr><tr>
                <td v-if="field('Description')?.screen !== false">Monthly Support</td>
                <td v-if="field('Quantity')?.screen !== false">1</td>
                <td v-if="field('Rate')?.screen !== false">$145.00</td>
                <td v-if="field('Amount')?.screen !== false">$145.00</td>
              </tr></tbody>
            </table>

            <div class="totals">
              <div v-if="field('Subtotal')?.screen !== false"><span>{{ field('Subtotal')?.title || 'Subtotal' }}</span><strong>$840.00</strong></div>
              <div v-if="field('Tax')?.screen !== false"><span>{{ field('Tax')?.title || 'Tax' }}</span><strong>$109.20</strong></div>
              <div v-if="field('Payments / Credits')?.screen !== false"><span>{{ field('Payments / Credits')?.title || 'Payments / Credits' }}</span><strong>$0.00</strong></div>
              <div v-if="field('Balance Due')?.screen !== false || field('Total')?.screen !== false" class="grand"><span>{{ field('Balance Due')?.screen !== false ? (field('Balance Due')?.title || 'Balance Due') : (field('Total')?.title || 'Total') }}</span><strong>$949.20</strong></div>
            </div>

            <div class="invoice-footer">
              <span v-if="field('Notes')?.screen !== false">Thank you for your business.</span>
              <span v-if="field('Terms & Conditions')?.screen !== false">CAD · Terms & Conditions</span>
              <span v-if="field('Print Date')?.screen !== false">Printed September 16, 2026</span>
              <span v-if="field('Page Numbers')?.screen !== false">Page 1 of 1</span>
            </div>
          </article>
        </div>
  ...
  methods: {
    ...
    async loadCustomizationFromTemplate(name: string) {
      ...
      const visible = (section: string) => {
        ...
        return !node || (node.getAttribute('data-cb-screen') ?? 'visible') !== 'hidden';
      };
      ...
      for (const field of Object.values(this.fields).flat()) {
        ...
        if (section) {
          field.screen = visible(section);
          field.print = (() => {
            const node = document.body.querySelector(`[data-cb-section="${section}"]`) as HTMLElement | null;
            return !node || (node.getAttribute('data-cb-print') ?? 'visible') !== 'hidden';
          })();
          const node = document.body.querySelector(
            `[data-cb-section="${section}"]`
          ) as HTMLElement | null;
        ...
        if (column) {
          ...
          field.screen = !cell || (cell.getAttribute('data-cb-screen') ?? 'visible') !== 'hidden';
          field.print = !cell || (cell.getAttribute('data-cb-print') ?? 'visible') !== 'hidden';
    ...
    async getUniqueTemplateName(baseName: string): Promise<string> {
      let name = baseName;
      let suffix = 2;
      while (await fyo.db.exists(ModelNameEnum.PrintTemplate, name)) {
    ...
    async copyPreset(name: string) {
      ...
      const copyName = await this.getUniqueTemplateName(`${name} Invoice`);
    ...
    async usePreset(name: string) {
      ...
      const doc = fyo.doc.getNewDoc(ModelNameEnum.PrintTemplate, {
        name: await this.getUniqueTemplateName(`${name} Invoice`),
        type: ModelNameEnum.SalesInvoice,
        template,
        isCustom: true,
      });
    ...
    customizeTemplate(template: string): string {
      ...
      for (const [section, visible] of Object.entries(sectionMap)) {
        ...
        if (node) {
          node.setAttribute('data-cb-screen', visible ? 'visible' : 'hidden');
          node.setAttribute('data-cb-print', visible ? 'visible' : 'hidden');
          node.style.display = '';
        }
      }
      ...
      for (const field of fields) {
        ...
        if (section) {
          ...
          node.setAttribute('data-cb-screen', field.screen ? 'visible' : 'hidden');
        ...
        if (column) {
          ...
          cells.forEach((cell) => {
            (cell as HTMLElement).setAttribute('data-cb-screen', field.screen ? 'visible' : 'hidden');
            (cell as HTMLElement).setAttribute('data-cb-print', field.print ? 'visible' : 'hidden');
